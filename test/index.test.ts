...
describe("flattenObject", () => {
    it("flattens nested object", () => {
        ...
        expect(flattenObject(input)).toEqual({ "a.b.c": 1, d: 2 });
    });

    it("respects separator", () => {
        const input = { a: { b: { c: 1 } } };
        expect(flattenObject(input, { separator: "_" })).toEqual({ "a_b_c": 1 });
    });

    it("keeps arrays by default", () => {
        const input = { a: [{ x: 1 }, { x: 2 }] };
        expect(flattenObject(input)).toEqual({ a: [{ x: 1 }, { x: 2 }] });
    });

    it("expands arrays when enabled", () => {
        const input = { a: [{ x: 1 }, { y: 2 }] };
        expect(flattenObject(input, { expandArrays: true })).toEqual({
            "a.0.x": 1,
            "a.1.y": 2,
        });
    });
